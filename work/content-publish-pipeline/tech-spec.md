...
- **`infrastructure/secrets/secrets.sops.yml`** *(extend)* — add content-publisher keys: `blogger_telegram_bot_token`, `blogger_telegram_channel`, `publish_min_score`, `publish_approval_timeout_min`, `publish_mode`, `blogger_repo_ref`, `claude_blog_repo_ref`, `publish_callback_hmac_secrets`. The HMAC value supports two comma-separated keys (`<current>,<previous>`) for non-disruptive rotation. `blogger_prompts_topic_id` is a backward-compatible fallback only; new deploys use the `telegram-init` generated `telegram_topics['blogger-prompts']`.
- **`infrastructure/ansible/playbooks/deploy.yml`** *(extend)* — add `blogger-prompts` to the `telegram-init` topic list, then insert `- role: content-publisher` between `fabric-services` (line 253) and `telegram-ai-agent` (line 290), passing `blogger_prompts_topic_id` from `telegram_topics['blogger-prompts']`.
...
**Decision:** Operator interacts with the pipeline exclusively inside a dedicated Telegram forum topic `📝 blogger-prompts`. CI/CD creates that topic through the existing `telegram-init` role, persists its `message_thread_id` in `/etc/fabric/telegram-topics.yml` and `infrastructure/inventory/telegram-topics.yml`, and passes `telegram_topics['blogger-prompts']` to `BLOGGER_PROMPTS_TOPIC_ID`. The old sops `blogger_prompts_topic_id` key is only a backward-compatible recovery fallback. Bot's aiogram handler matches `message.chat.id == TELEGRAM_FORUM_CHAT_ID AND message.message_thread_id == BLOGGER_PROMPTS_TOPIC_ID AND message.from_user.id == OPERATOR_USER_ID`; matching messages are enqueued as publish briefs directly (the full `message.text` becomes the `prompt`). NO `/publish_content` slash command. NO LLM in the dispatch path. NO MCP tool. Bot's preview reply lives in the same topic as a `reply_to_message_id=<brief_msg_id>` message; approval edits the preview in-place to "Published: <link>".
...
| `telegram-init` cannot create `📝 blogger-prompts` or the generated topic mapping is missing | Deploy fails before `content-publisher` is configured, because `telegram-init` owns topic creation and writes the `message_thread_id` mapping used for `BLOGGER_PROMPTS_TOPIC_ID`. The sops `blogger_prompts_topic_id` key remains only as a legacy/manual recovery fallback. |
...
- **Description:** Add content-publisher keys to `infrastructure/secrets/secrets.sops.yml.template`; keep `blogger_prompts_topic_id` only as a legacy fallback because Decision 13 now uses the `telegram-init` generated `telegram_topics['blogger-prompts']` mapping. Add empty placeholders to encrypted `secrets.sops.yml` via `sops edit`. Expose as Ansible facts via fabric-services sops load.
- **Skill:** code-writing
