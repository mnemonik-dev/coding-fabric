...
Если в проекте настроен coverage (проверить `pyproject.toml` / `pytest.ini` каждого пакета):

```bash
cd fabric/content-publisher && pytest --cov=content_publisher --cov-report=term-missing
cd mnemonik-bridge-workspace/telegram-ai-agent && pytest --cov=telegram_bot --cov-report=term-missing
```

Сравнить coverage feature-файлов с baseline из patterns.md / pyproject.toml. Падение → `critical`.
...
**Зависимости:**
- Tasks 9 (Code Audit), 10 (Security Audit), 11 (Test Audit) — должны быть `done`. Их JSON-отчёты в `work/content-publish-pipeline/logs/working/task-{9,10,11}/` дают вход для cross-reference: если test-audit (Task 11) уже отметил отсутствие теста для AC-T7, эта задача должна это поднять в свой отчёт, а не игнорировать.
...
- При множестве `not_verifiable` AC (ожидаемо для этой фичи — AC2/AC8/AC11/AC15/AC-T2/AC-T3/AC-T4) — отчёт всё равно может быть `status: passed`, если все остальные `passed` и тестовый сьют зелёный. Эти AC переходят в scope Task 14.
