...
- Operator forgets to create the `📝 blogger-prompts` forum topic, or puts the wrong `BLOGGER_PROMPTS_TOPIC_ID` in sops → the bot's topic-handler in Task 8 would never fire. Smoke: call `getForumTopic` (Bot API 7.2+) with `chat_id = TELEGRAM_FORUM_CHAT_ID` and `message_thread_id = BLOGGER_PROMPTS_TOPIC_ID` and assert `200 OK` + `result.ok == true`. `getForumTopicIconStickers` takes ZERO parameters and only returns globally-available icon stickers — it cannot validate that a specific topic exists in a specific chat.
...
1. **Append** to `infrastructure/ansible/roles/content-publisher/tasks/main.yml` (do NOT rewrite — Task 3's scaffold tasks stay in place above):
   ...
   - Smoke task A — channel-admin: HTTP `POST` to `https://api.telegram.org/bot{{ blogger_telegram_bot_token }}/getMe` to discover `publisher_bot_id = result.json.result.id`, then `POST` to `.../getChatMember` with `chat_id=@mnemonik` and `user_id={{ publisher_bot_id }}`. Non-200, `ok: false`, or `result.result.status not in ['administrator', 'creator']` → fail with a clear message naming the channel-admin runbook (`Publisher bot is not an admin of @mnemonik — see roles/content-publisher/README.md`).
   - Smoke task B — topic exists: HTTP `POST` to `https://api.telegram.org/bot{{ blogger_telegram_bot_token }}/getForumTopic` (Bot API 7.2+) with `chat_id={{ telegram_forum_chat_id }}` and `message_thread_id={{ blogger_prompts_topic_id }}`. Non-200 or `ok: false` → fail with a clear message naming the topic-creation runbook.
...
- A targeted topic-missing rehearsal: temporarily set `blogger_prompts_topic_id` to a non-existent thread id → the `getForumTopic` smoke task MUST fail with the named topic-creation message.
...
- [ ] Smoke task A calls `getMe` then `getChatMember` on `@mnemonik` with `user_id=<publisher_bot_id>` using `{{ blogger_telegram_bot_token }}`; assertion checks `result.result.status in ['administrator', 'creator']`; failure path emits a clear "publisher bot is not channel admin" message naming the role README runbook.
- [ ] Smoke task B calls `getForumTopic` (Bot API 7.2+) against `{{ telegram_forum_chat_id }}` + `{{ blogger_prompts_topic_id }}`; failure path emits a clear "topic missing or wrong id" message naming the role README runbook.
...
- [ ] Post-deploy: `systemctl is-active content-publisher` → `active`; `journalctl -u content-publisher --since "$(systemctl show -p ActiveEnterTimestamp content-publisher --value)"` contains all three start lines; topic + channel smokes pass.
...
- Negative rehearsals: bad publisher token (or bot not admin) → `getChatMember` smoke task fails with named runbook message; bad topic id → `getForumTopic` smoke task fails with named runbook message.
...
- During the play itself: `getMe` + `getChatMember` against `@mnemonik` with `{{ blogger_telegram_bot_token }}` returns `ok: true` AND `result.result.status in ['administrator', 'creator']`; `getForumTopic` for `BLOGGER_PROMPTS_TOPIC_ID` returns `ok: true` (both gated inside the role and fail-fast the deploy on miss).

## Details

**Files:**
- `infrastructure/ansible/roles/content-publisher/tasks/main.yml` — **APPEND** new tasks after Task 3's scaffold tasks. Order at the bottom of the file: (a) install package into venv, (b) `notify: reload systemd` triggers added to env/mcp.json/unit template tasks (Task 3's tasks), (c) enable + start `content-publisher.service`, (d) capture `ActiveEnterTimestamp` via `command: systemctl show -p ActiveEnterTimestamp content-publisher --value` register `cp_active_ts`, (e) `wait_for` / shell-grep against `journalctl --since "{{ cp_active_ts.stdout }}"` for the three log lines (30 s budget), (f) `getMe` + `getChatMember @mnemonik` admin-check smoke, (g) `getForumTopic` topic-exists smoke. Do NOT rewrite or reorder Task 3's tasks above.
...
- Telegram API rate limiting on the smoke calls: `getMe` + `getChatMember` + `getForumTopic` are three single calls per deploy — well under any rate ceiling. No retry/backoff needed; non-200 → fail-loud.
...
- Use `ansible.builtin.uri` module for the three Telegram smoke calls (`getMe`, `getChatMember`, `getForumTopic`). Body format: JSON; method POST; `status_code: 200`; `body_format: json`; `return_content: yes`. Register the result and assert `result.json.ok == true` (and for the channel-admin check, additionally `result.json.result.status in ['administrator', 'creator']`) with a custom `fail` task carrying the named runbook message — this keeps the failure surface readable in Ansible output.
