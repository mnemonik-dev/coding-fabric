...
The `📝 blogger-prompts` topic-existence class is explicitly NOT validated at deploy time. The Bot API has no read endpoint that takes a `message_thread_id` and confirms a specific topic exists in a specific chat — `getForumTopicIconStickers` returns globally-available icon stickers (zero parameters); `getForumTopic` (singular) does not exist in the Bot API at all. Per tech-spec Risks row 11, this misconfiguration is self-detected at first use: the bot's topic-handler in Task 8 logs the mismatched `message_thread_id` to journald, and the operator debugs via `journalctl -u telegram-ai-agent | grep thread_id` within minutes of the failed `/publish_content` attempt. Do NOT add an Ansible smoke for topic existence — there is no real API to call.
...
1. **Append** to `infrastructure/ansible/roles/content-publisher/tasks/main.yml` (do NOT rewrite — Task 3's scaffold tasks stay in place above):
   ...
   - Do NOT add a topic-existence smoke. The Bot API has no endpoint that confirms a specific `message_thread_id` exists in a specific chat. Topic-id mismatch is self-detected by Task 8's topic-handler at first use via journald (operator debugs with `journalctl -u telegram-ai-agent | grep thread_id`). The role README's runbook section already covers this debugging path.
...
- [ ] NO topic-existence smoke is added (no real Bot API endpoint exists for it). Tech-spec Risks row 11: topic-id mismatch is self-detected at first use via journald.
...
- [tech-spec.md](../tech-spec.md) — see `#### Task 7` and Risks row 11 ("Operator forgets to create topic OR puts wrong topic_id in sops" — explicitly no deploy-time smoke; self-detected via journald) and the channel-admin row ("Operator forgets to add publisher bot as channel admin" — `getChatMember`-based smoke)
...
**Files:**
- `infrastructure/ansible/roles/content-publisher/tasks/main.yml` — **APPEND** new tasks after Task 3's scaffold tasks. Order at the bottom of the file: (a) install package into venv, (b) `notify: reload systemd` triggers added to env/mcp.json/unit template tasks (Task 3's tasks), (c) enable + start `content-publisher.service`, (d) capture `ActiveEnterTimestamp` via `command: systemctl show -p ActiveEnterTimestamp content-publisher --value` register `cp_active_ts`, (e) `wait_for` / shell-grep against `journalctl --since "{{ cp_active_ts.stdout }}"` for the three log lines (30 s budget), (f) `getMe` + `getChatMember @mnemonik` admin-check smoke. Do NOT rewrite or reorder Task 3's tasks above. Do NOT add a topic-existence smoke — there is no real Bot API endpoint that takes a thread_id and confirms a topic exists; the misconfiguration is self-detected at first use via journald (tech-spec Risks row 11).
...
- Topic id correctness is NOT validated by this task. The Bot API has no read endpoint that takes a `message_thread_id`. If `blogger_prompts_topic_id` is wrong in sops, the failure surfaces at Task 8's topic-handler when the operator first writes in the wrong-resolved topic — the handler logs the inbound `message_thread_id` and the operator runs `journalctl -u telegram-ai-agent | grep thread_id` to compare against the configured value. The role README documents this debugging path.
...
- Do NOT call `getForumTopic` or `getForumTopicIconStickers` for topic validation. The former does not exist in the Bot API; the latter takes zero parameters and returns globally-available icon stickers (cannot confirm a specific topic exists in a specific chat). Tech-spec Risks row 11 documents the journald-debug fallback path that replaces a deploy-time topic smoke.
