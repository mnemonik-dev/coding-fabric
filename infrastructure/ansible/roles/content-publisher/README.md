...
| `blogger_prompts_topic_id` | generated by `telegram-init` as `telegram_topics['blogger-prompts']`; sops fallback only | — (required) |
...
The `blogger-prompts` topic is created by the `telegram-init` role during
deploy and persisted in `/etc/fabric/telegram-topics.yml` plus
`infrastructure/inventory/telegram-topics.yml`. The content-publisher role
receives `blogger_prompts_topic_id` from that generated mapping. The legacy
sops key is fallback only for recovering a pre-existing manually-created topic.
