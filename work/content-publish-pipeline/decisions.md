...
Task 11 (Test Audit) — re-audit by task-11-test-audit (test-master) after the remediation pass: verdict=`fail` (strict rubric — any weak AC fails). 5 findings across 28 ACs (26 covered, 2 weak, 0 missing); severity 0 high + 4 medium + 1 low. The remediation commits (telegram-ai-agent `4a29a97` + outer-repo `cfb635c`) closed the four originally-high findings (foreign-HMAC rejection, replay protection, Decision-9 from_user-before-HMAC ordering invariant, topic-filter 3-dimensional negatives + positive case) — verified by direct file re-read of `test_publish_handlers.py` (400 lines, 13 tests). Also closed: AC-T3 queue file mode 0600 (test_append_job_creates_queue_file_mode_0600); AC8 fire_at semantics (test_queue_poll_scheduling.py covering future-skipped + due-processed). Remaining weak ACs: AC4 (no [🔄] retry-button presence/absence assertion in bot preview-dispatch — reflexive reply_markup), AC6 (no reject-callback test for the [✗ Отклонить] branch). All 9 targeted checks pass except check_1 (3 medium/low findings on bot-side coverage gaps); checks 2–9 all PASS including HMAC/replay/ordering (check_5), topic filter (check_8), cleanup-gc (check_6), just-retry crash recovery (check_7), MCP-stdio handshake (check_9), preview-render cross-comparison (check_3). AC14 + AC-T13 accepted as jointly satisfied per Decision 8 / Task 11 prep. The remaining 3 actionable findings (F-001 reject-callback test, F-003 retry-button presence/absence, F-004 README runbook static check) are a tightly-scoped patch round for the bot test layer; recommend pre-deploy QA proceeds in parallel. Full report → [logs/audit/test-audit.json](logs/audit/test-audit.json).

Task 11 remediation (F-001 / F-003 / F-004) — ad-hoc fixer `fixer-bot-test-gaps` closed all three actionable Task 11 findings in a single test-reviewer round (verdict=`approve`). F-001 (AC6 reject-callback branch) — added `test_reject_callback_transitions_preview_to_rejected` in `mnemonik-bridge-workspace/telegram-ai-agent/tests/test_publish_handlers.py` (nested-repo commit `1f50103`); pins CAS `preview-sent → REJECTED`, asserts no transition to PUBLISHING and no `regenerate_job` call, and verifies the preview message is edited to the canonical "Отклонено. Бриф сохранён, можно повторить руками." with `reply_markup=None`. F-003 (AC4 retry-button presence/absence) — replaced the previously reflexive `test_preview_dispatch_sends_reply_and_clears_preview_pending` with two non-reflexive sibling tests that parse `InlineKeyboardMarkup` structurally via a `_callback_actions()` helper: low score (72) asserts the action list is exactly `["approve","reject","retry"]`, high score (90) asserts `["approve","reject"]` with `"retry" not in actions` — a flipped `score < min_score` conditional in `_preview_keyboard` is now caught. F-004 (AC-T11 README HMAC rotation runbook) — added `fabric/content-publisher/tests/test_readme_hmac_rotation_runbook.py` (2 tests, outer-repo commit `e50fcf4`) that opens `infrastructure/ansible/roles/content-publisher/README.md` and asserts the `## Runbook: rotate HMAC callback secret` heading plus the `CALLBACK_HMAC_SECRETS` two-secret format example exist; a README rewrite that drops the section now fails at test time. Verification: telegram-ai-agent pytest 91 passed (was 89); fabric/content-publisher pytest 107 passed + 1 skipped (was 105 + 1); ruff lint + format-check clean on both. Test-reviewer round-1 report → [logs/working/adhoc-bot-test-gaps/test-reviewer-round1.json](logs/working/adhoc-bot-test-gaps/test-reviewer-round1.json).

## Task 12: Pre-deploy QA

**Status:** Done
**Agent:** qa-runner
**Summary:** Pre-deploy gate **PASSED**. Both test suites green (`fabric/content-publisher` 107 passed + 1 importorskip-guarded skip; `mnemonik-bridge-workspace/telegram-ai-agent` 91 passed — 198 tests total, zero failures). All 28 acceptance criteria walked (15 user-spec + 13 tech-spec): 18 `passed` with explicit test/code evidence, 10 `not_verifiable` (live-environment scope rolling to Task 14 — AC7 MCP recall round-trip, AC16 telegram-init topic, AC-T2 service regression, AC-T3/T4 live mode/umount checks, plus partial deferrals on AC3/AC6/AC11/AC12/AC15 timing/auth/restart aspects). AC9 (past-date `--at` rejection) marked `not_verifiable` per Decision 13 — scheduled-publish deferred to follow-up feature, NOT Task 14; gate explicitly does not block on this. Zero `failed`, zero criticals/majors/minors. Coverage on content-publisher 76% overall (gaps concentrated in long-running orchestration paths — main.py 52%, deadline.py 65%, render.py 33% — all structurally covered by integration tests or AST-pins). Every Files-to-modify entry from Tasks 1–8 has at least one test or verify-only static check.
**Deviations:** None. Skipped byte-equality preview-render test (`test_preview_segments_byte_equal_to_publish_segments`) is loud-guarded via `pytest.importorskip('mnemonik_blogger.content.ingest')` because the upstream snapshot is not installed in the dev venv — by design, Decision 11's Ansible signature check at deploy time IS the gate, and two sibling AST-pin tests covering the same import surface DO pass here.

**Verification:**
- `fabric/content-publisher/.venv/bin/python -m pytest -v` → 107 passed, 1 skipped (0.25s)
- `mnemonik-bridge-workspace/telegram-ai-agent/.venv/bin/python -m pytest -v` → 91 passed (1.28s)
- `fabric/content-publisher/.venv/bin/python -m pytest --cov=content_publisher --cov-report=term-missing` → 76% overall
- Full report → [logs/pre-deploy-qa.json](logs/pre-deploy-qa.json)
