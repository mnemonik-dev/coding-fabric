...
Расширяем sops-инфраструктуру под content-publish-pipeline: фундамент для всех остальных волн. Добавляем 9 новых секретов/конфигов в `infrastructure/secrets/secrets.sops.yml.template` (документированный шаблон под `cp ... && sops -e -i ...`), кладём пустые/placeholder-значения в зашифрованный `secrets.sops.yml` через `sops edit`, и пробрасываем их как Ansible facts в playbook `deploy.yml` тем же способом, что и существующие vault_* ключи (см. `playbooks/deploy.yml` строки 50-102 — это **единственное место** в репозитории, где sops грузится через `community.sops.load_vars: name: sops` и мапится в `vault_*` через `set_fact`; роль `fabric-services` сама sops не грузит — она потребляет уже-готовые `vault_*` facts).

Решение #13 из tech-spec вводит **topic-based dispatch** вместо slash-команды: оператор пишет в forum-топик `📝 blogger-prompts` в существующем forum-чате. Поэтому в списке 9 ключей есть `blogger_prompts_topic_id` — message_thread_id этого топика, без него bot-handler не сможет фильтровать сообщения (`F.message_thread_id == BLOGGER_PROMPTS_TOPIC_ID`).
...
1. **Расширить `infrastructure/secrets/secrets.sops.yml.template`** — добавить новый блок-секцию (например `# === Content Publisher ===`) с 9 ключами и человекочитаемыми комментариями для каждого:
...
2. **Расширить зашифрованный `infrastructure/secrets/secrets.sops.yml`** — через `sops infrastructure/secrets/secrets.sops.yml` (открывает $EDITOR с расшифрованной копией; сохранение → автоматически шифрует обратно). Добавить те же 9 ключей с **placeholder-значениями** (НЕ настоящими секретами):
   - токены / SHA / hmac — пустая строка `""` (default-friendly; роли используют `| default('')`).
...
3. **Расширить `infrastructure/ansible/playbooks/deploy.yml`** — в существующий `set_fact`-блок "Expose sops keys as flat vault_* facts" (строки 62-102) добавить 9 новых `vault_*`-маппингов с `default('')` / `default(0)` / `default('approval')` где уместно. Сохранить `no_log: true` на уровне task. Не создавать отдельный set_fact — расширить существующий, чтобы все sops-факты жили в одном месте (паттерн).

## TDD Anchor

Sanity-проверка (не unit-тест в `pytest` — задача чисто конфигурационная, runtime-кода нет):

- **`sops -d infrastructure/secrets/secrets.sops.yml | python3 -c "import sys, yaml; d=yaml.safe_load(sys.stdin); req=['blogger_telegram_bot_token','blogger_telegram_channel','blogger_prompts_topic_id','publish_min_score','publish_approval_timeout_min','publish_mode','blogger_repo_ref','claude_blog_repo_ref','publish_callback_hmac_secrets']; missing=[k for k in req if k not in d]; assert not missing, missing"`** — расшифрованный YAML парсится и все 9 ключей присутствуют (пустые placeholder-значения допустимы).
- **`grep -E '^(blogger_telegram_bot_token|blogger_telegram_channel|blogger_prompts_topic_id|publish_min_score|publish_approval_timeout_min|publish_mode|blogger_repo_ref|claude_blog_repo_ref|publish_callback_hmac_secrets):' infrastructure/secrets/secrets.sops.yml.template`** — все 9 ключей задокументированы в template (`wc -l` == 9).
- **`ansible-playbook --syntax-check infrastructure/ansible/playbooks/deploy.yml`** — playbook валиден после правок `set_fact`.

## Acceptance Criteria

- [ ] В `secrets.sops.yml.template` присутствуют все 9 ключей с поясняющими комментариями (источник значения, формат, ссылка на BotFather/openssl/RawDataBot где уместно).
- [ ] В зашифрованном `secrets.sops.yml` присутствуют те же 9 ключей с placeholder/empty-значениями; реальные токены НЕ закоммичены.
...
- [ ] В `playbooks/deploy.yml` "Expose sops keys" `set_fact` расширен 9 новыми `vault_*`-маппингами с разумными `default(...)`; `no_log: true` сохранён.
...
- [project.md](../../../.claude/skills/project-knowledge/project.md) *(если существует — обзор coding-fabric)*
- [architecture.md](../../../.claude/skills/project-knowledge/architecture.md) *(если существует — карта компонентов)*
- [CLAUDE.md](../../../CLAUDE.md) — раздел Infrastructure + sops-file false-positive в GitGuardian
...
- [infrastructure/ansible/playbooks/deploy.yml](../../../infrastructure/ansible/playbooks/deploy.yml) — строки 50-102: `community.sops.load_vars` + `set_fact` маппинг `sops.* → vault_*`
- [infrastructure/ansible/roles/fabric-services/tasks/main.yml](../../../infrastructure/ansible/roles/fabric-services/tasks/main.yml) — роль НЕ грузит sops сама; потребляет уже-готовые `vault_*` (паттерн, который Task 3 будет повторять для content-publisher)
...
### Automated
- `sops -d infrastructure/secrets/secrets.sops.yml > /tmp/decrypted.yml && python3 -c "import yaml; d=yaml.safe_load(open('/tmp/decrypted.yml')); req=['blogger_telegram_bot_token','blogger_telegram_channel','blogger_prompts_topic_id','publish_min_score','publish_approval_timeout_min','publish_mode','blogger_repo_ref','claude_blog_repo_ref','publish_callback_hmac_secrets']; missing=[k for k in req if k not in d]; print('MISSING:', missing) or (None if not missing else (_ for _ in ()).throw(AssertionError(missing)))"` → нет missing.
...
- `infrastructure/secrets/secrets.sops.yml.template` — добавить блок-секцию `# === Content Publisher ===` с 9 ключами и комментариями (источник значения, формат, ссылки на @BotFather / `openssl rand -hex 32` / @RawDataBot для thread_id). Текущее состояние: 111 строк, документирует все существующие sops-секреты — формат секций уже устоявшийся (`# === <name> ===` + многострочные `# `-комментарии перед каждым ключом).
...
- Маппинг в `set_fact` блока deploy.yml читать как один длинный YAML-словарь. Новые ключи добавлять под существующими, сохраняя выравнивание двоеточий (как в текущем файле; см. строки 70-101). Пример нового маппинга:
  ```yaml
  ...
  ```
  (Это hint про стилистику — не финальный список; смотреть по факту все 9 ключей.)
...
- [ ] Обновить user-spec/tech-spec если что-то изменилось (например, если 9-й ключ оказался лишним или потребовался 10-й — синхронизировать tech-spec Decision 13 / Acceptance Criteria).
