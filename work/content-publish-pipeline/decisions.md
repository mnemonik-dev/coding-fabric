...
Task 11 (Test Audit): verdict=fail; report found 5 high gaps and 1 medium gap, mostly in bot-side HMAC negative/replay coverage, full topic-filter coverage, scheduled persistence/past-date coverage, runtime file-mode/runbook checks, and local mock drift; full report -> [logs/audit/test-audit.json](logs/audit/test-audit.json).

Task 11 remediation pass: Added focused regression coverage for the highest-risk runtime gaps without rewriting the audit artifact. Nested `telegram-ai-agent` commit `4a29a97` covers bot topic guard dimensions, enqueue behavior, foreign-user callback rejection, invalid HMAC rejection, and replay/stale-status callback behavior. This repo now covers queue file mode `0600` and `fire_at` queue polling so future scheduled jobs are not processed early. Verification: `telegram-ai-agent` pytest 89 passed; content-publisher pytest 105 passed, 1 skipped; touched-file ruff clean. Remaining product decision: the current Telegram topic flow has no scheduling syntax parser, so AC9-style past-date rejection cannot be tested until that interface exists.
